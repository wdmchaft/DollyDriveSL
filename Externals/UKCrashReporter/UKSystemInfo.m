...
void	UKGetSystemVersionComponents( long* outMajor, long* outMinor, long* outBugfix )
{
	...
	{
		Gestalt( gestaltSystemVersionMajor, (SInt32 *)outMajor );
		Gestalt( gestaltSystemVersionMinor, (SInt32 *)outMinor );
		Gestalt( gestaltSystemVersionBugFix, (SInt32 *)outBugfix );
	}
	else
	{
...
NSString*	UKMachineName()
{
	...
	{
		NSString*	internalName = [NSString stringWithCharacters:(const unichar *)(machineName +1) length: machineName[0]];
		...
		if( foundKey != nil && [foundKey rangeOfString: @","].location == NSNotFound )
