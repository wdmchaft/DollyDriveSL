...
//  CloneVolume
//
//  Created by Pumptheory P/L on 12/04/11.
...
// This has been shamelessly pulled from the Kernel sources

uint16_t latin_lower_table[] = {
...
static uint16_t lower_table[] = {

  /* High-byte indices ( == 0 iff no case mapping and no ignorables ) */
  
  /* 0 */	0x0000, 0x0100, 0x0000, 0x0200, 0x0300, 0x0400, 0x0000, 0x0000,
		0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
  /* 1 */	0x0500, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
		0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
  /* 2 */	0x0600, 0x0700, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
		0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
...
static inline uint16_t lower (uint16_t c)
{
  if (c < 0x100)
    return latin_lower_table[c];
  
  uint16_t base = lower_table[c >> 8];
  return base ? lower_table[base + (c & 0xff)] : c;
}

static inline uint16_t decode_lower (const char **p, uint16_t *next)
{
  uint32_t v;
  do {
    uint8_t c = **p;
    ...
    if (!c)
      ...
      v |= *++*p & 0x3f;
      ++*p;
      // This is a surrogate pair so split this into two
      v -= 0x0010000;
      *next = (v & 0x3ff) + 0xdc00;
      v = (v >> 10) + 0xd800;
    } else
...
int case_insens_cmp (const char *a, const char *b)
{
  uint16_t c1, c2, next_c1 = 0, next_c2 = 0;

  do {
    c1 = decode_lower (&a, &next_c1);
    c2 = decode_lower (&b, &next_c2);
  } while (c1 == c2 && c1);
