...
@implementation DSDisk

@synthesize diskRef;

+ (DSDisk *)diskWithDiskRef:(DADiskRef)aDiskRef
{
...
+ (DSDisk*)diskWithMountPath:(NSString*)mountPath
{
    ...
    if ((statfs([mountPath UTF8String], &buf)) == 0)
    {
        ...
        DSDisk* cvDisk = [[[self class] alloc] initWithDiskRef:disk];
...
- (DSDisk *)initWithDiskRef:(DADiskRef)aDiskRef
{
    if ((self = [super init])) {
        CFRetain (aDiskRef);
        diskRef = aDiskRef;
...
{
    return ([object isKindOfClass:[DSDisk class]]
...
void DSmountCallback(DADiskRef disk, 
...
void DS_disk_rename_callback(DADiskRef disk,
                          DADissenterRef dissenter,
                          void *context)
{
...
- (BOOL)renameTo:(NSString *)newName
{
    ...
    DADiskRename(diskRef, (CFStringRef)newName, kDADiskRenameOptionDefault, DS_disk_rename_callback, &ctx);
...
- (BOOL)mount
{
    ...
    DADiskMount (diskRef, NULL, 0, DSmountCallback, &ctx);
...
- (BOOL)unmount
{
    ...
    DADiskUnmount (diskRef, 0, DSmountCallback, &ctx);
...
- (NSString *)description
{
    ...
    return [NSString stringWithFormat:@"<DSDisk: %p %s>", self, bsdName];
}
