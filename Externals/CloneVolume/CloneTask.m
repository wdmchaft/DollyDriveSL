...
NSString *CVErrorDomain2 = @"CVErrorDomain";
...
@implementation CloneTask

@synthesize formatting;
@synthesize aborted;
@synthesize paused;
...
- (BOOL)Pause
{
  if (!self.formatting && !self.paused)
  {
...
-(void)getSettings
{
  ...
  if ([filemgr fileExistsAtPath: filePathTarget] == YES)
  {
    ...
    NSLog(@"Last run = %@", [plistDict objectForKey:@"NextRunDate"]);
...
- (void)startCloneThread   
{
  ...
  NSString *launchPath = [NSString stringWithString:@"/Library/Application Support/DollyClone/CloneVolume Helper"];// [[ADDAppConfig sharedAppConfig] cloneHelperPath];
